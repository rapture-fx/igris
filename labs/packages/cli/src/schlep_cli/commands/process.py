"""
Data processing commands for Schlep-engine CLI.
"""
...
def file(ctx, file_path: str, output: Optional[str], output_format: Optional[str], 
    ...
    Examples:
        schlep process file data.csv
        schlep process file data.csv --output processed.parquet --format parquet
        schlep process file data.csv --clean --profile
    """
...
def batch(ctx, pattern: str, output_dir: Optional[str], output_format: Optional[str],
    ...
    Examples:
        ...
        schlep process batch "data/*.csv" --parallel 8 --clean
        schlep process batch "**/*.json" --output-dir processed --format parquet
    """
    config = ctx.obj.get('config') or Config()
    client = ctx.obj.get('client')
...
def upload(ctx, file_path: str, bucket: Optional[str], key: Optional[str], 
    ...
    Examples:
        ...
        schlep process upload data.csv --bucket my-bucket --key data/input.csv
        schlep process upload data.csv --public --metadata project=demo
    """
    config = ctx.obj.get('config') or Config()
    client = ctx.obj.get('client')
...
def download(ctx, file_id: str, output: Optional[str]):
    ...
    Examples:
        schlep process download file-12345
        schlep process download file-12345 --output downloaded_data.csv
    """
    config = ctx.obj.get('config') or Config()
    client = ctx.obj.get('client')
