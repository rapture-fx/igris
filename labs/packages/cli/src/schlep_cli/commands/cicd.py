"""
CI/CD pipeline integration commands for Schlep-engine CLI.
...
def setup_pipeline(ctx, project_path: str, provider: str, template: str, 
    ...
    """
    Setup CI/CD pipeline for a project with Schlep-engine integration.
    
    Examples:
        schlep cicd setup-pipeline ./my-project --provider github --template ml-training
        schlep cicd setup-pipeline ./api --provider gitlab --environments dev staging prod
        schlep cicd setup-pipeline ./data --provider jenkins --triggers push schedule
    """
...
def trigger(ctx, pipeline_config: str, environment: Optional[str], branch: Optional[str],
    ...
    Examples:
        schlep cicd trigger .github/workflows/deploy.yml --environment production
        schlep cicd trigger .gitlab-ci.yml --branch main --wait
        schlep cicd trigger pipeline.yml --parameters "VERSION=1.2.3" "ENV=staging"
    """
    config = ctx.obj.get('config') or Config()
    client = ctx.obj.get('client')
...
def list_runs(ctx, provider: Optional[str], status: Optional[str], limit: int, watch: bool, json: bool):
    ...
    Examples:
        schlep cicd list-runs --provider github --status running
        schlep cicd list-runs --limit 50 --watch
        schlep cicd list-runs --json
    """
    config = ctx.obj.get('config') or Config()
    client = ctx.obj.get('client')
...
def status(ctx, run_id: str, provider: Optional[str], logs: bool, json: bool):
    ...
    Examples:
        schlep cicd status 123456789 --provider github
        schlep cicd status run-abc123 --logs
        schlep cicd status 987654321 --json
    """
    config = ctx.obj.get('config') or Config()
    client = ctx.obj.get('client')
...
def package(ctx, source_dir: str, output: Optional[str], include_config: bool,
    ...
    Examples:
        schlep cicd package ./src --output deployment.zip --compress
        schlep cicd package ./app --include-config --exclude "*.log" "*.tmp"
    """
    config = ctx.obj.get('config') or Config()
...
def validate(ctx, test_configs: tuple, provider: Optional[str], fix_issues: bool, output_format: str):
    ...
    Examples:
        schlep cicd validate .github/workflows/*.yml --provider github
        schlep cicd validate .gitlab-ci.yml --fix-issues
        schlep cicd validate pipeline1.yml pipeline2.yml --output-format json
    """
    config = ctx.obj.get('config') or Config()
...
def load_pipeline_template(template: str, config_file: Optional[str]) -> Dict[str, Any]:
    ...
    templates = {
        'data-pipeline': {
            'name': 'Schlep-engine Data Pipeline',
        ...
        'ml-training': {
            'name': 'Schlep-engine ML Training',
            'environments': ['dev', 'staging', 'production'],
        ...
        'api-deployment': {
            'name': 'Schlep-engine API Deployment',
            'environments': ['dev', 'staging', 'production'],
            'triggers': ['push'],
            'steps': [
        ...
        'batch-processing': {
            'name': 'Schlep-engine Batch Processing',
...
def generate_github_actions_workflow(project_dir: Path, config: Dict[str, Any]) -> List[Path]:
    ...
    workflow_config = {
        'name': config.get('name', 'Schlep-engine Pipeline'),
        'on': {},
        'jobs': {}
    }
    ...
    # Write workflow file
    workflow_file = workflows_dir / 'schlep-engine-pipeline.yml'
...
pipeline {{
    ...
    environment {{
        SCHLEP_ENGINE_API_KEY = credentials('schlep-engine-api-key')
    }}
    ...
    post {
        ...
        failure {
            emailext body: 'Pipeline failed', subject: 'Schlep-engine Pipeline Failure', to: '$DEFAULT_RECIPIENTS'
        }
    }
}
"""
...
def generate_circleci_config(project_dir: Path, config: Dict[str, Any]) -> List[Path]:
    ...
    for env in config.get('environments', ['dev']):
        ...
        circleci_config['workflows']['schlep-engine-pipeline']['jobs'].append({
            job_name: {
                'context': f'schlep-engine-{env}',
                'filters': {
                    'branches': {
                        'only': ['main'] if env == 'production' else ['develop', 'main']
                    }
...
def convert_step_to_github_action(step: Dict[str, Any], env: str) -> Dict[str, Any]:
    ...
    step_mapping = {
        ...
        'install-dependencies': {
            ...
            'run': 'pip install -r requirements.txt && pip install schlep-engine-cli'
        },
        'validate-data': {
            'name': 'Validate data',
            'run': 'schlep process batch "data/*.csv" --validate --dry-run'
        },
        'process-data': {
            'name': 'Process data',
            'run': f'schlep process batch "data/*.csv" --parallel 4 --output-dir processed_{env}'
        },
        'train-model': {
            'name': 'Train ML model',
            'run': f'schlep pipeline create config/ml-training-{env}.yml --auto-start'
        },
        'deploy-pipeline': {
            'name': 'Deploy pipeline',
            'run': f'schlep devops deploy infrastructure/ --env {env}'
        }
    }
...
def convert_step_to_gitlab_script(step: Dict[str, Any], env: str) -> str:
    ...
    script_mapping = {
        ...
        'install-dependencies': 'pip install -r requirements.txt && pip install schlep-engine-cli',
        'validate-data': 'schlep process batch "data/*.csv" --validate --dry-run',
        'process-data': f'schlep process batch "data/*.csv" --parallel 4 --output-dir processed_{env}',
        'train-model': f'schlep pipeline create config/ml-training-{env}.yml --auto-start',
        'deploy-pipeline': f'schlep devops deploy infrastructure/ --env {env}'
    }
...
def convert_step_to_jenkins_script(step: Dict[str, Any]) -> str:
    ...
    script_mapping = {
        ...
        'install-dependencies': 'sh "pip install -r requirements.txt && pip install schlep-engine-cli"',
        'validate-data': 'sh "schlep process batch \\"data/*.csv\\" --validate --dry-run"',
        'process-data': 'sh "schlep process batch \\"data/*.csv\\" --parallel 4"',
        'train-model': 'sh "schlep pipeline create config/ml-training.yml --auto-start"',
        'deploy-pipeline': 'sh "schlep devops deploy infrastructure/ --env ${env}"'
    }
...
def convert_step_to_azure_task(step: Dict[str, Any], env: str) -> Dict[str, Any]:
    ...
    task_mapping = {
        ...
        'install-dependencies': {
            'script': 'pip install -r requirements.txt && pip install schlep-engine-cli',
            'displayName': 'Install dependencies'
        },
        'validate-data': {
            'script': 'schlep process batch "data/*.csv" --validate --dry-run',
            'displayName': 'Validate data'
        },
        'process-data': {
            'script': f'schlep process batch "data/*.csv" --parallel 4 --output-dir processed_{env}',
...
def convert_step_to_circleci_step(step: Dict[str, Any], env: str) -> Dict[str, Any]:
    ...
    step_mapping = {
        ...
        'install-dependencies': {
            'run': {
                'name': 'Install dependencies',
                'command': 'pip install -r requirements.txt && pip install schlep-engine-cli'
            }
        },
        'validate-data': {
            'run': {
                'name': 'Validate data',
                'command': 'schlep process batch "data/*.csv" --validate --dry-run'
            }
        },
        'process-data': {
            'run': {
                'name': 'Process data',
                'command': f'schlep process batch "data/*.csv" --parallel 4 --output-dir processed_{env}'
            }
        }
    }
...
def show_pipeline_next_steps(provider: str, files: List[Path]):
    ...
    if provider == 'github':
        ...
        next_steps_text.append("2. Add SCHLEP_ENGINE_API_KEY to your repository secrets\n", style="white")
        ...
        next_steps_text.append("2. Add SCHLEP_ENGINE_API_KEY as a CI/CD variable\n", style="white")
        ...
        next_steps_text.append("2. Create 'schlep-engine-api-key' credential in Jenkins\n", style="white")
