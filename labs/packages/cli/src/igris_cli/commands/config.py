"""
Configuration management commands for Igris-engine CLI.
"""
...
def init(ctx, config_file: Optional[str]):
    ...
    Examples:
        igris config init
        igris config init --config-file ~/.igris/custom-config.yml
    """
    try:
        config_obj = Config()
...
def list(ctx, json: bool, config_file: Optional[str]):
    ...
    Examples:
        igris config list
        igris config list --json
        igris config list --config-file custom-config.yml
    """
    try:
...
def get(ctx, key: str, config_file: Optional[str]):
    ...
    Examples:
        ...
        igris config get parallel_jobs
        igris config get api_key --config-file custom-config.yml
    """
    try:
        config_obj = Config.load(config_file)
...
def set(ctx, key: str, value: str, config_file: Optional[str], value_type: Optional[str]):
    ...
    Examples:
        ...
        igris config set base_url https://api.custom.com
        igris config set auto_clean true --type bool
        igris config set timeout 60 --type int
    """
    try:
        config_obj = Config.load(config_file)
...
def unset(ctx, key: str, config_file: Optional[str]):
    ...
    Examples:
        igris config unset parallel_jobs
        igris config unset base_url
    """
    try:
        config_obj = Config.load(config_file)
...
def update(ctx, settings: tuple, config_file: Optional[str]):
    ...
    Examples:
        ...
        igris config update base_url=https://api.custom.com show_progress=false
    """
    if not settings:
        console.print("[red]No settings provided. Use key=value format.[/red]")
        console.print("Example: igris config update parallel_jobs=8 timeout=60")
        sys.exit(1)
...
def reset(ctx, config_file: Optional[str], force: bool):
    ...
    Examples:
        igris config reset
        igris config reset --force
        igris config reset --config-file custom-config.yml
    """
    try:
        if not force:
...
def validate(ctx, config_file: Optional[str]):
    ...
    Examples:
        igris config validate
        igris config validate --config-file custom-config.yml
    """
    try:
        config_obj = Config.load(config_file)
        ...
        if not config_obj.api_key:
            validation_results.append({
                ...
                'severity': 'error',
                'suggestion': 'Run: igris auth login'
            })
