"""
Pipeline management commands for Igris-engine CLI.
"""
...
def create(ctx, config_file: str, name: Optional[str], description: Optional[str],
    ...
    Examples:
        ...
        igris pipeline create model-config.json --name "My Pipeline" --auto-start
        igris pipeline create config.yml --dry-run  # Validate only
    """
    ...
    with multi_step_spinner(steps, "Creating pipeline") as spinner:
        try:
            ...
            if auto_start and pipeline_id:
                ...
                console.print(f"Pipeline ID: [cyan]{pipeline_id}[/cyan]")
                console.print("Use 'igris pipeline start' to begin execution")
...
def list(ctx, status: str, limit: int, json: bool):
    ...
    Examples:
        ...
        igris pipeline list --limit 20 --json
    """
    config = ctx.obj.get('config') or Config()
    client = ctx.obj.get('client')
...
def status(ctx, pipeline_name_or_id: str, json: bool, watch: bool):
    ...
    Examples:
        igris pipeline status my-pipeline
        igris pipeline status pl-123456 --json
        igris pipeline status my-pipeline --watch  # Live updates
    """
    config = ctx.obj.get('config') or Config()
    client = ctx.obj.get('client')
...
def start(ctx, pipeline_name_or_id: str, force: bool):
    ...
    Examples:
        ...
        igris pipeline start pl-123456 --force
    """
    config = ctx.obj.get('config') or Config()
    client = ctx.obj.get('client')
...
def stop(ctx, pipeline_name_or_id: str, force: bool):
    ...
    Examples:
        igris pipeline stop my-pipeline
        igris pipeline stop pl-123456 --force
    """
    config = ctx.obj.get('config') or Config()
    client = ctx.obj.get('client')
...
def logs(ctx, pipeline_name_or_id: str, lines: int, follow: bool, level: Optional[str]):
    ...
    Examples:
        igris pipeline logs my-pipeline
        igris pipeline logs pl-123456 --lines 500 --follow
        igris pipeline logs my-pipeline --level ERROR
    """
    config = ctx.obj.get('config') or Config()
    client = ctx.obj.get('client')
...
def delete(ctx, pipeline_name_or_id: str, force: bool):
    ...
    Examples:
        ...
        igris pipeline delete pl-123456 --force
    """
    config = ctx.obj.get('config') or Config()
    client = ctx.obj.get('client')
