"""
DevOps automation commands for Igris-engine CLI.
...
def deploy(ctx, infrastructure_dir: str, env: str, parallel: int, validate_only: bool, 
    ...
    Examples:
        igris devops deploy ./infrastructure --env staging
        igris devops deploy ./k8s --parallel 5 --rollback-on-failure
        igris devops deploy ./terraform --validate-only
    """
...
def batch_config(ctx, config_pattern: str, batch_size: int, parallel: int,
    ...
    Examples:
        igris devops batch-config "configs/*.yml" --batch-size 5
        igris devops batch-config "k8s/**/*.yaml" --validate --parallel 8
        igris devops batch-config "terraform/*.tf" --template-vars env=prod region=us-east-1
    """
    config = ctx.obj.get('config') or Config()
...
def status(ctx, provider: Optional[str], namespace: Optional[str], watch: bool, output_format: str):
    ...
    Examples:
        igris devops status --provider kubernetes
        igris devops status --provider kubernetes --namespace production --watch
        igris devops status --provider docker --output-format json
    """
    config = ctx.obj.get('config') or Config()
...
def health_check(ctx, service_configs: tuple, health_check_url: Optional[str],
    ...
    Examples:
        igris devops health-check service1.yml service2.yml
        igris devops health-check configs/*.yml --health-check-url "https://{service}.example.com/health"
        igris devops health-check --parallel-checks 10 --max-retries 5
    """
