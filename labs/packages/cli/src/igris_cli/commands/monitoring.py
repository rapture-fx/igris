"""
Monitoring and analytics commands for Igris-engine CLI.
"""
...
def status(ctx, json: bool, detailed: bool):
    ...
    Examples:
        igris monitoring status
        igris monitoring status --detailed
        igris monitoring status --json
    """
...
def metrics(ctx, json: bool, watch: bool, interval: int):
    ...
    Examples:
        igris monitoring metrics
        igris monitoring metrics --watch
        igris monitoring metrics --json
    """
    config = ctx.obj.get('config') or Config()
    client = ctx.obj.get('client')
...
def jobs(ctx, status: str, limit: int, json: bool):
    ...
    Examples:
        igris monitoring jobs
        igris monitoring jobs --status running
        igris monitoring jobs --limit 50 --json
    """
    config = ctx.obj.get('config') or Config()
    client = ctx.obj.get('client')
...
def job(ctx, job_id: str, json: bool, watch: bool):
    ...
    Examples:
        igris monitoring job job-123456
        igris monitoring job job-123456 --watch
        igris monitoring job job-123456 --json
    """
    config = ctx.obj.get('config') or Config()
    client = ctx.obj.get('client')
...
def cancel_job(ctx, job_id: str, force: bool):
    ...
    Examples:
        igris monitoring cancel-job job-123456
        igris monitoring cancel-job job-123456 --force
    """
    config = ctx.obj.get('config') or Config()
    client = ctx.obj.get('client')
