"""
Main CLI entry point for Igris-engine CLI tool.
"""
...
def cli(ctx, version: bool, debug: bool, config_file: Optional[str]):
    """
    Igris-engine CLI - Advanced data processing and ML automation
    ...
    Examples:
        ...
        igris process data.csv --output processed.parquet
        igris pipeline create fraud-detection.yml
        igris batch process ./input/*.csv --parallel 4
    """
...
def show_version():
    ...
    version_info = Text()
    version_info.append("Igris-engine CLI ", style="bold cyan")
...
def show_welcome():
    ...
    welcome_text = Text()
    welcome_text.append("Welcome to Igris-engine CLI!\n\n", style="bold cyan")
    ...
    welcome_text.append("igris auth login --api-key YOUR_API_KEY\n", style="green")
    welcome_text.append("  2. ", style="dim")
    welcome_text.append("igris process batch \"data/*.csv\" --parallel 4\n", style="green")
    welcome_text.append("  3. ", style="dim")
    welcome_text.append("igris pipeline create fraud-detection.yml --auto-start\n", style="green")
    
    welcome_text.append("\n🛠️ DevOps & Automation:\n", style="bold")
    welcome_text.append("  • ", style="dim")
    welcome_text.append("igris devops deploy infrastructure/ --env production\n", style="cyan")
    welcome_text.append("  • ", style="dim")
    welcome_text.append("igris cicd setup-pipeline . --provider github --template ml-training\n", style="cyan")
    welcome_text.append("  • ", style="dim")
    welcome_text.append("igris batch submit-files \"data/*.csv\" --parallel-jobs 8\n", style="cyan")
    
    welcome_text.append("\n📊 Monitoring:\n", style="bold")
    welcome_text.append("  • ", style="dim")
    welcome_text.append("igris monitoring status --detailed\n", style="yellow")
    welcome_text.append("  • ", style="dim")
    welcome_text.append("igris devops status --provider kubernetes --watch\n", style="yellow")
    
    welcome_text.append("\nUse ", style="dim")
    welcome_text.append("igris --help", style="bold")
...
def health(ctx):
    ...
    if not client:
        console.print("[red]No API client configured. Run 'igris auth login' first.[/red]")
        sys.exit(1)
