/**
 * Main Igris-engine JavaScript SDK Client
 * Official TypeScript/JavaScript client for the Igris-engine API platform
 */
...
import { IgrisError, ConfigurationError } from '../utils/errors';
...
export const SDK_INFO = {
  name: 'Igris-engine JavaScript SDK',
  version: '1.0.0',
  company: 'Igris-engine',
  description: 'Official JavaScript/TypeScript SDK for Igris-engine API',
  documentation: 'https://docs.igris-inertial.com/sdk/javascript',
  support: 'https://support.igris-inertial.com',
  github: 'https://github.com/igris-inertial/javascript-sdk'
} as const;

/**
 * Main Igris-engine SDK client class
 * Provides access to all Igris-engine API endpoints with built-in authentication,
 * retry logic, and error handling
 */
export class IgrisClient extends EventEmitter {
  // Core components
  ...
  /**
   * Initialize Igris-engine client
   * 
   * @example
   * ```typescript
   * // With API key
  ...
  constructor(config: SDKConfig = {}) {
    ...
    this.config = {
      baseUrl: config.baseUrl || 'https://api.igris-inertial.com',
    ...
    if (this.config.debug) {
      console.log('Igris-engine client initialized:', {
        baseUrl: this.config.baseUrl,
  ...
  async close(): Promise<void> {
    try {
      ...
      if (this.config.debug) {
        console.log('Igris-engine client closed');
      }
    } catch (error) {
      if (this.config.debug) {
