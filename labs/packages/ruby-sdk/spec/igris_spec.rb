...
RSpec.describe Igris::Engine::Client do
  ...
  describe '#initialize' do
    ...
    it 'raises ConfigurationError with nil API key' do
      expect { described_class.new(nil) }.to raise_error(Igris::Engine::ConfigurationError)
    end

    it 'raises ConfigurationError with empty API key' do
      expect { described_class.new('') }.to raise_error(Igris::Engine::ConfigurationError)
    end
  end

  describe '.from_env' do
    it 'creates client from environment variable' do
      ENV['IGRIS_API_KEY'] = 'env-api-key'
      client = described_class.from_env(base_url: base_url)
      expect(client).to be_a(described_class)
      ENV.delete('IGRIS_API_KEY')
    end

    it 'raises ConfigurationError when env var not set' do
      ENV.delete('IGRIS_API_KEY')
      expect { described_class.from_env }.to raise_error(Igris::Engine::ConfigurationError)
    end
  end
  ...
  describe '#upload' do
    it 'successfully uploads data' do
      ...
      expect(result).to be_a(Igris::Engine::UploadResponse)
  ...
  describe '#train' do
    it 'successfully trains with hash config' do
      ...
      expect(result).to be_a(Igris::Engine::TrainResponse)
  ...
  describe '#deploy' do
    it 'successfully deploys model' do
      ...
      expect(result).to be_a(Igris::Engine::DeployResponse)
      expect(result.deployment_id).to eq('deploy_101')
      expect(result.endpoint_url).to eq('https://api.igris-inertial.com/models/model_789/predict')
  ...
  describe '#status' do
    it 'successfully checks job status' do
      ...
      expect(result).to be_a(Igris::Engine::StatusResponse)
  ...
  describe 'error handling' do
    it 'raises ApiError on 401 Unauthorized' do
      ...
      expect { client.upload('test data') }.to raise_error(Igris::Engine::ApiError) do |error|
    ...
    it 'raises ApiError on 400 Bad Request' do
      ...
      expect { client.train({}) }.to raise_error(Igris::Engine::ApiError) do |error|
    ...
    it 'handles network errors' do
      ...
      expect { client.upload('test data') }.to raise_error(Igris::Engine::NetworkError)
    end
  end
end