"""
Thin Client for Igris Inertial API
...
class IgrisError(Exception):
    """Base exception for Igris Inertial thin client"""
    pass
...
class ValidationError(IgrisError):
...
class ClientConfig(BaseModel):
    ...
    user_agent: str = Field("igris-thin-client/1.0.0", description="User agent string")
...
class IgrisThinClient:
    ...
    def __init__(self, base_url: str, api_key: Optional[str] = None, **config_overrides):
        ...
        config_dict = {
            "base_url": base_url,
            "api_key": api_key or os.getenv("IGRIS_API_KEY"),
        }
...
def create_client(base_url: str, api_key: Optional[str] = None, **kwargs) -> IgrisThinClient:
    """Create thin client instance"""
    return IgrisThinClient(base_url, api_key, **kwargs)

# Default client for environment variables
def default_client() -> IgrisThinClient:
    """Create client using environment variables"""
    return IgrisThinClient(
        base_url=os.getenv("IGRIS_API_URL", "https://api.igris-inertial.com"),
        api_key=os.getenv("IGRIS_API_KEY"),
        timeout=int(os.getenv("IGRIS_TIMEOUT", "30"))
    )
