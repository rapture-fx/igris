"""
Enhanced logging utilities for Igris-engine SDK with correlation IDs and structured logging
"""
...
def get_logger(name: str, contextual: bool = False) -> Union[logging.Logger, ContextualLogger]:
    ...
    """
    # Ensure all SDK loggers are under the igris namespace
    if not name.startswith("igris"):
        name = f"igris.{name}"
...
class LoggerMixin:
    ...
    def logger(self) -> logging.Logger:
        ...
        if module_name.startswith("igris."):
            name = f"{module_name}.{class_name}"
        else:
            name = f"igris.{class_name}"
