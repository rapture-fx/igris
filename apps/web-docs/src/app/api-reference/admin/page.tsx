  ...
  const codeExamples = [
    {
      language: 'curl',
      label: 'cURL',
      code: `# Get system statistics
curl -X GET "https://api.schlep-engine.com/api/v1/admin/system/stats" \\
  -H "Authorization: Bearer sk_your_admin_key"

# Create user with admin privileges
curl -X POST "https://api.schlep-engine.com/api/v1/admin/users" \\
  -H "Authorization: Bearer sk_your_admin_key" \\
...
api_key = "sk_your_admin_key"
headers = {"Authorization": f"Bearer {api_key}"}

# Get system statistics
stats_response = requests.get(
    "https://api.schlep-engine.com/api/v1/admin/system/stats",
    headers=headers
)
...
if stats_response.status_code == 200:
    ...
    print(f"📊 Total API calls today: {stats['api_calls_today']}")
    ...
    user_response = requests.post(
        "https://api.schlep-engine.com/api/v1/admin/users",
        headers=headers,
        json=user_data
    )
    ...
    },
    {
      language: 'javascript',
      label: 'JavaScript',
      code: `const apiKey = 'sk_your_admin_key';

// Get system statistics
async function getSystemStats() {
  try {
    const response = await fetch('https://api.schlep-engine.com/api/v1/admin/system/stats', {
      headers: {
    ...
    if (response.ok) {
      ...
      console.log(\`📊 Total API calls today: \${stats.api_calls_today}\`);
      
      // Create admin user
      const userResponse = await fetch('https://api.schlep-engine.com/api/v1/admin/users', {
        method: 'POST',
        headers: {
          'Authorization': \`Bearer \${apiKey}\`,
