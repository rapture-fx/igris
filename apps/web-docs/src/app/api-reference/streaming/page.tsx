...
wscat -c "wss://api.schlep-engine.com/ws?token=sk_your_api_key"

# Create streaming data connection
curl -X POST "https://api.schlep-engine.com/api/v1/data-streaming/connections/" \\
...
async def setup_streaming():
    ...
    response = requests.post(
        "https://api.schlep-engine.com/api/v1/data-streaming/connections/",
        headers=headers,
    ...
    if response.status_code == 201:
        ...
        uri = f"wss://api.schlep-engine.com/ws?token={api_key}&connection_id={connection_id}"
            ...
            while True:
                try:
                    ...
                    if data['type'] == 'data_update':
                        print(f"📊 New data: {data['payload']['record_count']} records")
                    elif data['type'] == 'quality_alert':
                        print(f"⚠️ Quality alert: {data['payload']['message']}")
