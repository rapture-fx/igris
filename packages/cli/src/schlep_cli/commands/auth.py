"""
Authentication commands for Schlep-engine CLI.
"""
...
def login(ctx, api_key: Optional[str], interactive: bool):
    """
    Log in to Schlep-engine API.
    
    Examples:
        schlep auth login --api-key sk-1234567890abcdef
        schlep auth login --interactive
        schlep auth login  # Uses SCHLEP_API_KEY environment variable
    """
    config = ctx.obj.get('config') or Config()
    
    # Get API key
    if interactive:
        console.print("[cyan]Interactive login to Schlep-engine[/cyan]")
        api_key = Prompt.ask(
            "Enter your API key",
            password=True,
            show_default=False
    ...
    if not api_key:
        ...
        console.print("\nGet your API key at: https://dashboard.schlep-engine.com/api-keys")
        sys.exit(1)
    ...
    with operation_spinner("Authenticating", "Verifying API key...") as spinner:
        try:
            ...
            if success:
                ...
                success_text.append("\nYou can now use all Schlep-engine CLI commands.", style="dim")
...
def logout(ctx, confirm: bool):
    ...
    Examples:
        schlep auth logout
        schlep auth logout --confirm  # Skip confirmation
    """
    config = ctx.obj.get('config') or Config()
...
def status(ctx, json: bool):
    ...
    if config.is_authenticated():
        ...
        console.print(Panel(
            "[red]✗ Not authenticated[/red]\n\nRun 'schlep auth login' to authenticate.",
            title="Authentication Status",
            border_style="red"
        ))
    ...
    with operation_spinner("Refreshing authentication", "Validating current credentials...") as spinner:
        try:
            ...
            if is_healthy:
                ...
                console.print("Run: schlep auth login")
                sys.exit(1)
