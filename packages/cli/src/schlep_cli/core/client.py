"""
API client wrapper for Schlep-engine CLI.
"""
...
try:
    ...
    from schlep_engine.exceptions import SchlepEngineError, AuthenticationError, RateLimitError
    SDK_AVAILABLE = True
except ImportError:
    SDK_AVAILABLE = False
    SchlepEngineClient = None
    SchlepEngineError = Exception
...
class APIClient:
    """Wrapper for Schlep-engine SDK with CLI-specific functionality."""
    ...
    def __init__(self, config):
        ...
        if not SDK_AVAILABLE:
            console.print("[yellow]Warning: Schlep-engine SDK not available. Install with: pip install schlep-engine[/yellow]")
            return
        
        if config and config.api_key:
            try:
                self._client = SchlepEngineClient(
    ...
    @property
    def client(self) -> Optional[SchlepEngineClient]:
    ...
    def authenticate(self, api_key: str) -> bool:
        ...
        if not SDK_AVAILABLE:
            console.print("[red]Schlep-engine SDK not available[/red]")
            return False
        
        try:
            self._client = SchlepEngineClient(
                api_key=api_key,
                base_url=self.config.base_url if self.config else "https://api.schlep-engine.com",
    ...
    def process_file(self, file_path: Union[str, Path], **kwargs) -> Dict[str, Any]:
        ...
        if not self.is_authenticated:
            raise AuthenticationError("Not authenticated. Run 'schlep auth login' first.")
