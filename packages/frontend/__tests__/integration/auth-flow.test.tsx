...
describe('Authentication Flow Integration Tests', () => {
    ...
    it('handles password reset errors', async () => {
      ...
      server.use(
        rest.post('/api/v1/auth/password-reset-request', (req, res, ctx) => {
          return res(
            ctx.status(404),
            ctx.json({ detail: 'User not found' })
          );
        })
      );
    ...
    it('handles 2FA verification', async () => {
      ...
      server.use(
        rest.post('/api/v1/auth/2fa/verify', (req, res, ctx) => {
          return res(
            ctx.json({ message: '2FA verified successfully' })
          );
        })
      );
      ...
      // Wait for success message
      await waitFor(() => {
        expect(screen.getByText(/2fa verified successfully/i)).toBeInTheDocument();
      });
    });
  });
  ...
  describe('API Key Management', () => {
    it('creates new API key', async () => {
      ...
      // Enter API key name
      await user.type(screen.getByPlaceholderText(/api key name/i), 'Test API Key');
