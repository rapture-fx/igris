...
**Gold Code Override** is Igris Inertial's permanent control plane bypass mode that enables fully autonomous operation in air-gapped, edge, or zero-trust environments. When enabled, the SDK operates entirely independently using locally cached Thompson Sampling parameters.
...
export SCHLEP_FALLBACK_STRATEGY="thompson"
```
...
client = SchlepClient(api_key="your-api-key")
...
- Set via `SCHLEP_FALLBACK_STRATEGY=round-robin`
...
export BYOK_BYPASS_CONTROL_PLANE=true
export SCHLEP_CACHE_DIR="/secure/storage/schlep"
...
ENV SCHLEP_CACHE_DIR=/app/cache

COPY app /app/
COPY schlep_cache /app/cache/
...
~/.config/schlep/bayesian_state.enc
...
| `SCHLEP_CACHE_DIR` | `~/.config/schlep` | Cache directory path |
| `SCHLEP_FALLBACK_STRATEGY` | `thompson` | Fallback strategy (thompson\|round-robin) |
| `SCHLEP_EXPLORATION_RATE` | `0.10` | Exploration rate (0.0-1.0) |
| `SCHLEP_CACHE_TTL_HOURS` | `72` | Cache validity duration |
...
schlep-cli cache export --output bayesian_state.enc
...
cp ~/.config/schlep/bayesian_state.enc /backups/$(date +%Y%m%d).enc
```
...
- Email: enterprise@schlep-engine.com
