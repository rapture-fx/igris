...
1. Visit [console.schlep-engine.com](https://console.schlep-engine.com)
...
```bash
Authorization: Bearer sk-schlep-abc123...
```

### Key Format

```
sk-schlep-[40+ alphanumeric characters]
```

Example: `sk-schlep-1234567890abcdefghijklmnopqrstuvwxyz`
...
```bash
curl -X POST https://api.schlep-engine.com/v1/admin/api-keys \
...
```json
{
  "new_key": "sk-schlep-NEW_KEY_HERE",
...
```bash
curl -X POST https://api.schlep-engine.com/v1/auth/refresh \
...
```bash
curl -X POST https://api.schlep-engine.com/v1/auth/2fa/enable \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json"
```
...
export SCHLEP_API_KEY="sk-schlep-abc123..."

curl https://api.schlep-engine.com/v1/chat/completions \
  -H "Authorization: Bearer $SCHLEP_API_KEY"
```
...
```
Production:  sk-schlep-prod-abc123...
Staging:     sk-schlep-staging-xyz789...
Development: sk-schlep-dev-def456...
```
