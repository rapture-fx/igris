...
```bash
curl https://api.schlep-engine.com/v1/health \
...
```bash
curl https://api.schlep-engine.com/v1/health/metrics \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl https://api.schlep-engine.com/healthz
```
...
```bash
curl https://api.schlep-engine.com/readyz
```
...
```bash
curl https://api.schlep-engine.com/startupz
```
...
scrape_configs:
  - job_name: 'schlep-engine'
    scrape_interval: 15s
    static_configs:
      - targets: ['api.schlep-engine.com:8080']
