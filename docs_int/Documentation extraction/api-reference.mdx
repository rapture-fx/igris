...
Complete REST API reference for Igris Inertial. All endpoints are OpenAI-compatible with additional enterprise features.

## Base URL

```
Production:  https://api.igris-inertial.com
Staging:     https://api-staging.igris-inertial.com
...
```bash
curl https://api.igris-inertial.com/v1/infer \
...
```bash
# Login to get token
curl -X POST https://api.igris-inertial.com/v1/auth/login \
  -H "Content-Type: application/json" \
  -d '{
...
```json
{
  "error": {
    ...
    "documentation_url": "https://docs.igris-inertial.com/errors/invalid_api_key"
  }
}
```
...
```
https://api.igris-inertial.com/v1/infer
```
...
```javascript
const ws = new WebSocket('wss://api.igris-inertial.com/v1/stream');
...
- **Python**: `pip install igris-inertial`
- **JavaScript/TypeScript**: `npm install @igris-inertial/javascript-sdk`
- **Go**: `go get github.com/igris-inertial/go-sdk`
- **Rust**: `cargo add igris-inertial-sdk`
