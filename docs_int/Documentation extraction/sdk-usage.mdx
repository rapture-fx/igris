...
Schlep Engine provides official SDKs for **Go**, **Python**, and **OpenAI-compatible** clients. All SDKs support intelligent routing with Thompson Sampling and EscapeVector Mode for zero-downtime resilience.
...
```bash
go get github.com/schlep-engine/schlep-engine/internal/sdk/go/schlep
```
...
client := schlep.NewClient(&schlep.Config{
    ...
    APIKey:  os.Getenv("SCHLEP_API_KEY"),    // optional authentication
    Timeout: 60 * time.Second,                // default: 30s
})
```

**Environment Variables:**
- `SCHLEP_BASE_URL` - API base URL
- `SCHLEP_API_KEY` - API authentication key
...
if err != nil {
    if apiErr, ok := err.(*schlep.APIError); ok {
...
Schlep Engine is **OpenAI-compatible** - use any OpenAI SDK by changing the base URL.
...
# Point to Schlep Engine instead of OpenAI
client = OpenAI(
...
```go
// Go SDK
fmt.Println(schlep.SDKVersion) // "1.0.0-rc1"
```
