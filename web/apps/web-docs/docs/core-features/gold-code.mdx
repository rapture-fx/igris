...
**Gold Code Override** is Schlep Engine's permanent control plane bypass mode that enables fully autonomous operation in air-gapped, edge, or zero-trust environments. When enabled, the SDK operates entirely independently using locally cached Thompson Sampling parameters.
