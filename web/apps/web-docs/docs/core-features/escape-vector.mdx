...
When the control plane fails, Schlep Engine automatically switches to intelligent local routing. No round-robin fallback. No degraded performance. The same Thompson Sampling algorithm keeps running offline.
...
# Control plane goes down → EscapeVector activates automatically
...
```bash
export SCHLEP_CACHE_DIR="/var/cache/schlep"
```
...
EscapeVector is enabled by default. Just start using Schlep Engine:
