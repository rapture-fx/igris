...
**TL;DR:** Schlep Engine is a high-performance routing platform that sits between your app and LLM providers, intelligently selecting the best provider for each request.
...
1. **Your app makes a request** to Schlep Engine (same format as OpenAI)
...
The brain of Schlep Engine. Uses multiple algorithms to select the right provider:
...
[View on GitHub →](https://github.com/schlep-engine/schlep-engine)
