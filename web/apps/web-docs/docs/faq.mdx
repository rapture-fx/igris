...
Schlep Engine **automatically fails over** to the next best provider in <30 seconds. Your requests never fail.
...
Schlep Engine is an **AI routing and cost optimization platform** that sits between your app and multiple LLM providers. Instead of managing OpenAI, Anthropic, Google separately, you make one API call—Schlep Engine routes it to the best provider based on cost, speed, and quality.
...
Using Schlep Engine = automatic routing across OpenAI, Anthropic, Google, etc. with:
...
Schlep Engine is **100% OpenAI-compatible**. Use any OpenAI SDK (Python, Node.js, Go, etc.) with zero code changes beyond the base URL.
...
Your account will be paused. You can upgrade to Develop, Growth, or Scale tier anytime to continue using Schlep Engine.
...
No. **Scale tier is our highest public tier** with all features unlocked. For custom needs (air-gapped, white-label), contact sales@schlep-engine.com.
...
BYOK allows you to use **your own LLM provider API keys** instead of Schlep Engine's. Keys are:
...
Yes! Schlep Engine routes requests across all configured providers based on:
...
### What architecture does Schlep Engine use?

Schlep Engine uses a **hybrid polyglot architecture:**
...
Yes! Schlep Engine is a standard HTTP API:
...
### Where can I deploy Schlep Engine?

- **Cloud Hosted** - Managed by Schlep Engine (all tiers)
...
- **Documentation:** docs.schlep-engine.com
- **GitHub Issues:** github.com/schlep-engine/schlep-engine/issues
...
- GitHub Issues
- Email to support@schlep-engine.com
- Discord community
...
3. **Provider down** - Schlep Engine will auto-fallback to next provider
...
Yes! Schlep Engine is open source (MIT license). See [CONTRIBUTING.md](https://github.com/schlep-engine/schlep-engine/blob/main/CONTRIBUTING.md) for guidelines.
...
- GitHub: [Report an issue](https://github.com/schlep-engine/schlep-engine/issues)
