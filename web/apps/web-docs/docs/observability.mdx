...
Schlep Engine provides comprehensive observability:
...
```bash
curl https://api.schlep.cloud/metrics
```
...
curl https://api.schlep.cloud/v1/metrics/cost?group_by=provider

# Get cost metrics by tenant
curl https://api.schlep.cloud/v1/metrics/cost?group_by=tenant

# Get cost metrics by model
curl https://api.schlep.cloud/v1/metrics/cost?group_by=model
```
...
If you're using cloud hosted Schlep Engine, dashboards are built-in:
...
scrape_configs:
  - job_name: 'schlep-engine'
    static_configs:
      - targets: ['api.schlep.internal:8081']
...
# datadog.yaml
instances:
  - prometheus_url: http://api.schlep.internal:8081/metrics
    namespace: schlep_engine
    metrics:
      - '*'
```
...
- [Quick Start](/docs/quickstart) - Get started with Schlep Engine
