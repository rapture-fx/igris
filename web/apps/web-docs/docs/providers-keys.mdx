...
Schlep Engine supports **Bring Your Own Key (BYOK)** for multiple LLM providers. You can connect your existing provider accounts and manage API keys securely through our platform.
...
Via Dashboard:
1. Log in to https://app.schlep.cloud
...
Via API:
```bash
curl -X POST https://api.schlep.cloud/v1/tenants/{tenant_id}/providers/openai/keys \
  -H "Authorization: Bearer YOUR_SCHLEP_API_KEY" \
...
# Schlep Engine automatically routes to the best provider
...
Schlep Engine automatically selects the best provider based on performance, cost, and availability.
...
**Via API:**
```bash
curl -X POST https://api.schlep.cloud/v1/tenants/{tenant_id}/providers/openai/keys/rotate \
  -H "Authorization: Bearer YOUR_SCHLEP_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
**Solution:**
- Schlep Engine automatically fails over to other providers
...
**Solution:**
- Schlep Engine includes automatic retry logic
- Add more providers for better redundancy
- Check provider status pages for ongoing incidents
- Contact support@schlep.cloud if issues persist
...
```bash
GET https://api.schlep.cloud/v1/providers
```
...
```bash
POST https://api.schlep.cloud/v1/tenants/{tenant_id}/providers/{provider}/keys
```

### Rotate Provider Key

```bash
POST https://api.schlep.cloud/v1/tenants/{tenant_id}/providers/{provider}/keys/rotate
```
...
- [Routing Policies](/docs/routing-policies) - How Schlep Engine selects providers
