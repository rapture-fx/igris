...
**TL;DR:** Schlep Engine automatically selects the best AI provider for each request using Thompson Sampling (a smart learning algorithm). Growth+ tiers unlock Speculative Execution (60% faster) and Council Mode (+15-20% better quality).

---

## Overview

Schlep Engine uses intelligent routing to automatically select the best provider for each request based on:
...
1. Each provider starts with equal probability
2. After each request, the system records:
   - How fast the provider responded
   - How much the request cost
   - Whether it succeeded or failed
3. Over time, better providers get selected more often
4. The system keeps exploring alternatives to avoid missing improvements
...
```
score = (40% × speed) + (30% × cost-efficiency) + (30% × reliability)
```

Providers with higher scores are selected more frequently, but the system maintains 15% exploration to discover improvements.
</details>
...
1. Request comes in
2. Schlep Engine races 2-3 providers simultaneously
...
Schlep Engine analyzes each prompt and classifies it into categories:
...
```bash
POST https://api.schlep.cloud/v1/routing/preview
...
- **100 requests**: Basic preferences established
- **500 requests**: Stable routing patterns
