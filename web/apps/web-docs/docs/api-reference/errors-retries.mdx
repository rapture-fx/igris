...
**Fix:** Retry with exponential backoff. Schlep Engine automatically falls back to healthy providers.
...
**Fix:** Schlep Engine automatically routes to healthy providers. No action needed.
...
client = OpenAI(
    base_url="https://api.schlep-engine.com/v1",
...
Schlep Engine automatically handles provider failures:
...
client = OpenAI(base_url="https://api.schlep-engine.com/v1", api_key="...")
...
```python
# Schlep Engine automatically falls back to healthy providers
