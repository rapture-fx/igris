...
function Section({ bucket, tasks }: { bucket: Bucket; tasks: Task[] }) {
    ...
    >
      ...
      <div className="px-2 py-2">
        {rows.length === 0 ? (
          ...
          rows.map((t) => <Row key={t.task_id} task={t} />)
        )}
      </div>
    </div>
  );
...
export default function HomePage() {
  const { data, isLoading } = useTasks({ limit: 60 });
    ...
    <DashboardLayout>
      <div className="flex-1 overflow-y-auto">
        <div className="mx-auto max-w-[960px] px-8 py-10">
          ...
          <p className="text-[12px] text-[#7a7a72] mb-6">
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {BUCKETS.map((b) => <Section key={b.id} bucket={b} tasks={tasks} />)}
            </div>
          )}
