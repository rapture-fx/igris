      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="pb-4 border-b border-border-light">
            <h1 className="text-base font-medium text-gray-900 font-inter">Speculative Router</h1>
        ...
        <div className="bg-beige-primary">
          <div className="grid grid-cols-2 divide-x divide-border-light">
            ...
            <div className="p-4">
              ...
              <div className="text-lg font-bold text-gray-900">{status?.success_rate.toFixed(1)}%</div>
              <p className="text-[0.65rem] text-gray-600 mt-1">{status?.races_24h.toLocaleString()} races today</p>
            </div>
          </div>
          <div className="grid grid-cols-2 divide-x divide-border-light border-t border-border-light">
            <div className="p-4">
              <div className="text-xs font-medium text-gray-600 mb-1">Latency Gain</div>
              <div className="text-lg font-bold text-gray-900">{status?.latency_improvement_ms}ms</div>
              <p className="text-[0.65rem] text-gray-600 mt-1">average improvement</p>
            </div>
            <div className="p-4">
              <div className="text-xs font-medium text-gray-600 mb-1">Cost Delta</div>
              <div className="text-lg font-bold text-gray-900">+{status?.cost_delta_percent.toFixed(1)}%</div>
              <p className="text-[0.65rem] text-gray-600 mt-1">extra provider calls</p>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <Label htmlFor="enabled" className="text-xs font-medium text-gray-900">Enable Speculative Router</Label>
                    <Switch
                      id="enabled"
                  ...
                  <p className="text-[0.65rem] text-gray-600">Race providers in parallel</p>
                </div>

                <div className="space-y-2">
                  <Label htmlFor="max_parallel" className="text-xs font-medium text-gray-900">Max Parallel Providers</Label>
                  <Input
                    id="max_parallel"
                    type="number"
                    ...
                    max="5"
                  />
                  <p className="text-[0.65rem] text-gray-600">Providers to race simultaneously</p>
                </div>

                <div className="space-y-2">
                  <Label htmlFor="timeout" className="text-xs font-medium text-gray-900">Timeout (ms)</Label>
                  <Input
                    id="timeout"
                    type="number"
                    ...
                    className="text-xs"
                  />
                  <p className="text-[0.65rem] text-gray-600">Maximum race duration</p>
                </div>

                <div className="space-y-2">
                  <Label htmlFor="first_token" className="text-xs font-medium text-gray-900">First Token Threshold (ms)</Label>
                  <Input
                    id="first_token"
                    type="number"
                    ...
                    className="text-xs"
                  />
                  <p className="text-[0.65rem] text-gray-600">Winner selection cutoff</p>
                </div>
              </div>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-border-light">
                    ...
                    <th className="text-left py-2 px-3 font-medium text-gray-600">Winner</th>
                    <th className="text-left py-2 px-3 font-medium text-gray-600">Participants</th>
                    <th className="text-left py-2 px-3 font-medium text-gray-600">Latency Gain</th>
                    <th className="text-left py-2 px-3 font-medium text-gray-600">Cost Saving</th>
                  </tr>
                </thead>
                <tbody>
                  {races?.map((race, index) => (
                    <tr key={index} className="border-b border-gray-100 hover:bg-beige-secondary">
                      <td className="py-2 px-3 text-gray-900 font-mono">{race.request_id.slice(0, 12)}</td>
                      <td className="py-2 px-3">
                        <Badge className="bg-green-50 text-green-700 border border-green-200">
                          {race.winner}
                        </Badge>
                      </td>
                      <td className="py-2 px-3 text-gray-900">{race.participants.join(', ')}</td>
                      <td className="py-2 px-3 text-gray-900">{race.latency_gain_ms}ms</td>
                      <td className="py-2 px-3 text-gray-900">${race.cost_saving_usd.toFixed(3)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
