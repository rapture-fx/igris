...
export default function HeroInertial() {
  ...
  useEffect(() => {
    ...
    const coreGeometry = new THREE.SphereGeometry(1.45, 48, 48) // High segment count for compact grid
    ...
    innerCore.position.y = 0.2 // Move down to show top portion
    innerCoreRef.current = innerCore
    scene.add(innerCore)

    // Add diagonal grid overlay (rotated 45 degrees)
    const diagonalGeometry = new THREE.SphereGeometry(1.45, 48, 48)
    ...
    outerShell.position.y = 0.2 // Move down to match inner core
