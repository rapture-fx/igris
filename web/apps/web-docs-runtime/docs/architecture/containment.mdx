...
`Bounds::memory_mb` is communicated to the runtime via the `X-Igris-Bounds` request header from SDK clients. Kernel-level `memory.max` enforcement is not yet wired in the current release. The field exists in the data model and is forwarded correctly.
...
Bounds are forwarded to the runtime on every request via the `X-Igris-Bounds` HTTP header:

```
X-Igris-Bounds: {"cpu_percent":40,"memory_mb":512,"max_tick_ms":200}
```
