...
```mermaid
graph TD
    App["Your Application or Agent\n(SDK or HTTP client)"]
    App -->|"POST /v1/actions/:name/run"| Coordination
...
- **Actions API** — The primary request entry point: registered actions run through `/v1/actions/:name/run`, with `/v1/tasks` for durable task workflows and `/v1/mcp` for agents speaking MCP
- **Management API** — Runtime lifecycle, policy, receipts, vault keys, and operator workflows
- **Inference API (experimental)** — `/v1/infer` and `/v1/chat/completions`, disabled by default behind an explicit feature flag
