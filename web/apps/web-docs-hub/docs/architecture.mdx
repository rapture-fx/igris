...
| Capability | Purpose |
|---|---|
| **Adaptive Router** | Thompson Sampling, speculative execution, council mode, cost-aware routing, quality scoring, stream merging |
| **Circuit Breaker** | Per-provider failure detection with open/closed/half-open states |
| **Cognitive Advisor** | Periodic analysis of routing performance with optimization proposals |
...
| **Routing Engine** | Thompson Sampling, speculative execution, council mode |
...
<StepChain steps={[
  ...
  { label: "Coordination Layer", description: "Adaptive Router selects provider via Thompson Sampling, applies SLO and circuit breaker checks" },
