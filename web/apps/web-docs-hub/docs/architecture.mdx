...
<StepChain steps={[
  { label: "Application Request", description: "Initiates with a unique request ID (e.g. abc123)" },
  { label: "Coordination Layer", description: "Produces routing decision, tenant context, and selected provider" },
  { label: "Execution Engine", description: "Applies capability grants and wraps execution in a signed envelope" },
  { label: "Signed Receipt", description: "Canonical hash over inputs, outputs, timing, and resource usage" },
  { label: "Audit Log", description: "Queryable, exportable, tamper-evident — linked by request ID" }
]} />
