...
| Future canonical profile | RFC 003/008 | Python/Go agree on current fixtures; current Go diverges for U+2028/U+2029 | `igris-canonical-json-1` with exact parse/render rules | New schema only; legacy dispatch retained | Yes | YES | NO | NO | OD-02 resolved; implementation conformance kept separate |
...
| JSON numbers | RFC 003 | Python permits arbitrary ints/finite floats; Go can preserve literals | Future schemas use safe integers only; schema `1` external verification preserves original number lexemes | No existing bytes change; hostile spellings fail closed if lexeme unavailable | Future only | YES | NO | NO | Cross-language exactness over host normalization |
...
| Escaping | RFC 003 | Python raw U+2028/U+2029; current Go escapes them | Schema `1` pins Python raw UTF-8; future profile also specifies raw scalars | No historical byte change; later Go fix required | Future bytes only | YES | NO | NO | Known Go defect documented, not normalized |
...
| Verification-result schema | RFC 001/007 | Python/Go issue models differ | `igris:protocol:verification-result:1` decomposes all phases, including specification consistency | Existing verifiers need adapters, not byte changes | No signed artifact change | YES | NO | NO | OD-09 vocabulary reconciled; machine schema remains open |
| Event-type result | RFC 001/003/007 | Current implementations use unknown/unsupported labels | Unknown type in a supported closed schema is `invalid_field`; unknown schema is `unsupported_schema` | Removes overlapping portable alias only | No | YES | NO | NO | One issue meaning and dimension |
| Binding interval result | RFC 004/007 | Outside and unevaluable interval terms conflict | `outside_binding_interval` requires trustworthy time; `binding_interval_indeterminate` means time is insufficient | Preserves valid signature in both states | No | YES | NO | NO | Mutually exclusive trust states |
| Time-confidence vocabulary | RFC 004/007 | Draft used both `unknown` and `unavailable` | `unavailable` is sole no-usable-time value; basis values are not policy labels | Result draft only | No | YES | NO | NO | One portable enum |
| Specification conflict | RFC 000/001 | Issue existed without result dimension | `specification=conflict`, affected phases not evaluated, summary `indeterminate`, stop | No artifact reinterpretation | No | YES | NO | NO | Fail closed without implicit precedence |
