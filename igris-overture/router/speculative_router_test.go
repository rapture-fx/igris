...
func (m *MockProvider) Infer(ctx context.Context, req *models.InferRequest) (*models.InferResponse, error) {
	return nil, fmt.Errorf("non-streaming not implemented in mock")
}
	...
	go func() {
		...
		for i := 0; i < m.totalTokens; i++ {
			...
			chunk := models.NewStreamChunk(
				fmt.Sprintf("req_%s", m.name),
				"test-model",
				fmt.Sprintf("token_%d_%s ", i, m.name),
				0,
				"",
			)
...
func (m *MockProvider) EstimateCost(req *models.InferRequest) (float64, error) {
	return 0.001, nil
}
