...
func (st *SelfTuner) runTuningCycle(ctx context.Context) {
	log.Info().Msg("Starting self-tuning cycle")

	// Get all semantic classes
	classes, err := st.getSemanticClasses(ctx)
	if err != nil {
	...
	for _, class := range classes {
		result, err := st.optimizeClassWeights(ctx, class)
		if err != nil {
		...
		if result.ConfidenceScore >= st.confidenceThreshold && result.PerformanceImprovement > 0 {
			...
			log.Info().
				...
				Float64("improvement", result.PerformanceImprovement).
				Float64("confidence", result.ConfidenceScore).
				Msg("Weights optimized but not applied (low confidence or negative improvement)")
		}
	}
